...
Whisper.prototype.post = async function(topic, symKeyId, message) {
    ...
    try {
      ...
      // if symmetric key id exists, add it to the message option
      if(symKeyId != null) {
          messageOptions.symKeyID = symKeyId;
      }
 ...
 */
Whisper.prototype.subscribe = async function(topics, symKeyId, onMessage, onError) {
  ...
  try {
    ...
    // if symmetric key id exists, add it to the subscription option
    if(symKeyId != null) {
        subscriptionOptions.symKeyID = symKeyId;
    }
