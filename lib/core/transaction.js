...
Transaction.prototype.callContract = function(user, address, abi) {
    ...
    return new Promise((resolve, reject) =>  {
        _okdk._web3.eth.getTransactionCount(user.getAddressString())
        .then(result => {
            console.log("2.4");
             ccwv(user, result, address, 0, abi).then(result => {
                resolve(result);
             }).catch(error => {
                reject(error);
             });
 ...
 */
Transaction.prototype.callContractWithValues = function(user, nonce, to, value, data) {
