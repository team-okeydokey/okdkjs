 ...
 */
Houses.prototype.createListing = async function(houseName, hostName, 
    ...
    try {
        ...
        const newId = await this.registerHouse(bzzHash, gridId);
 ...
 */
Houses.prototype.registerHouse = async function(bzzHash, gridId) {

    try {
        var user = _okdk.accounts[0];
        var bytesHash = _okdk.web3.utils.asciiToHex(bzzHash);

        // Construct abi.
        var callData = this.contract.methods.registerHouse(bytesHash,gridId).encodeABI();
